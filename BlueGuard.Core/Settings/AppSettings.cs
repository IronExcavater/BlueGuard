...
namespace BlueGuard.Core.Settings;
...
public sealed class AppSettings(ISettingsStore store)
{
    ...
    public bool ReopenAfterToggle
    {
        get => store.Get(nameof(ReopenAfterToggle), defaultValue: true);
        set => store.Set(nameof(ReopenAfterToggle), value);
    }
}
